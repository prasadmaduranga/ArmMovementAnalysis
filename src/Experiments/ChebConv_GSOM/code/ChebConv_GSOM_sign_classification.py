...
model_archive_file = '../models/ChebConvLSTM_400sample_2023_02_11_V1.pt'
...
def TestMidgetter(model, mid_getter, test_dataloader):
    inputs, labels = next(iter(test_dataloader))
    [batch_size, step_size, fea_size] = inputs.size()
...
mid_getter = MidGetter(lstm, return_layers=return_layers, keep_output=True)

lstm_test, encoded_feats = TestMidgetter(lstm, mid_getter, all_dataloader)

# lstm_test,encoded_feats = TestMidgetter(lstm, mid_getter, test_dataloader)


np.savetxt(feature_encode_file, encoded_feats, delimiter=',', header='f1,f2,f3,f4,f5,f6,f7,f8,sign')
