...
def extract_hand_landmarks_from_images(args):
    with mp_pose.Pose(
        ...
        for root, directories, filenames in os.walk(args.image_input_dir):
            ...
            feature_list = ['IMAGE_ID'] + ['HANDEDNESS']+ left_hand_keypoint_headings + right_hand_keypoint_headings + body_keypoint_headings
...
def extract_frame_keypoints(image):
    ...
    if (handedness is not None):
        ...
        image_keypoints.append("'" + handedness + "'")
        ...
        for body_landmark in Body:
            ...
            image_keypoints.append({'X': results_pose.pose_landmarks.landmark[body_landmark.value].x,
...
def annotate_frame_keypoints(image):
    results_hand = video_hand_key_extractor.process(image)
    results_pose = video_pose_key_extractor.process(image)
